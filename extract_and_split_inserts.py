...
class InsertExtractorAndSplitter:
    ...
    def _generate_import_script(self, output_path, prefix, total_chunks):
        ...
        with open(py_script, 'w', encoding='utf-8') as f:
            ...
            f.write("def import_file(file_path, db_name, db_user, db_host, db_port):\n")
            f.write('    """导入单个SQL文件"""\n')
            f.write("    cmd = [\n")
            f.write("        'psql',\n")
            f.write("        '-h', db_host,\n")
            ...
            f.write("    \n")
            f.write("    try:\n")
            f.write("        result = subprocess.run(cmd, capture_output=True, text=True, check=True)\n")
            ...
            f.write("        futures = {\n")
            f.write("            executor.submit(import_file, f, args.database, args.user, args.host, args.port): f\n")
